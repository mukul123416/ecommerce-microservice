...
public class KafKaConsumerService {
    ...
    public KafKaConsumerService(KafKaProducerService kafKaProducerService, InventoryRepository inventoryRepository) {
    ...
    public void consume(String event) throws ExecutionException, InterruptedException {
        ...
        orderPriceCalDetails=this.gson.fromJson(event, OrderPriceCalDetails.class);
        Long productId = orderPriceCalDetails.getProductId();
        Inventory byProductId = this.inventoryRepository.findByProductId(productId);
        if (byProductId!=null) {
